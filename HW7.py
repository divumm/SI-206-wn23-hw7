...
# List who you have worked with on this project: Julia Coffmann
...
def make_players_table(data, cur, conn):
    ...
    cur.execute('CREATE TABLE IF NOT EXISTS Players (id INTEGER PRIMARY KEY, name INTEGER, position_id INTEGER, birthyear INTEGER, nationality TEXT)')
    ## how do you access position id
    for element in data['squad']:
        cur.execute("SELECT id FROM Positions WHERE position = ?", (element["position"],))
        position_id = int(cur.fetchone()[0])
        entry = "INSERT INTO Players(id, name, position_id, birthyear, nationality) VALUES(?,?,?,?,?)"
        index = (element['id'], element['name'], position_id, element['dateOfBirth'][:4], element['nationality'])
        cur.execute(entry, index)
    conn.commit() 
...
def make_winners_table(data, cur, conn):
    ...
    winners = []
    for winner in data['seasons']:
        if str(winner['winner']) != 'None':
            winners_ = winner['winner']['name']
            if winners_ not in winners:
                winners.append(winners_)
    #print(winners)
    cur.execute("CREATE TABLE IF NOT EXISTS Winners (id INTEGER PRIMARY KEY, winner_name TEXT UNIQUE)")
    for i in range(len(winners)):
        #print(winners[i])
        cur.execute("INSERT OR IGNORE INTO Winners (id, winner_name) VALUES (?,?)",(i, str(winners[i])))
    conn.commit()
...
def make_seasons_table(data, cur, conn):
    ...
    for team in data["seasons"]:
        #print(team)
        if str(team['winner']) != 'None':
            cur.execute("SELECT id FROM Winners WHERE winner_name = ?", (team["winner"]["name"], ))
            winner_id = int(cur.fetchone()[0])
            #print(cur.fetchall())
            entry = "INSERT INTO Seasons(id, winner_id, end_year) VALUES(?,?,?)"
            index = (team['id'], winner_id, team['endDate'][:4])
            cur.execute(entry, index)
    conn.commit()

def winners_since_search(year, cur, conn):
    d = {}
    cur.execute('SELECT Seasons.id, Winners.winner_name FROM Seasons JOIN Seasons ON Winners Seasons.winner_id = Winners.id WHERE Seasons.end_year > ?', (year, ))
